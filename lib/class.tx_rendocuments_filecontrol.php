...
/**
 * @desc Builds de file control used to manage the documents.
 * @author Pierrick Caillon <pierrick@in-cite.net>
 * @remarks Some code comes from TYPO3 core, t3lib_tceforms.
 */
class tx_rendocuments_filecontrol extends t3lib_TCEforms {
	/**
	 * @desc Génère le code du contrôle backend personnalisé d'envoie de fichier pour les documents.
	 * @author Pierrick Caillon <pierrick@in-cite.net>
	...
	function getSingleField_typeGroup($table,$field,$row,&$PA,$tce)	{
		...
		$size = 1;//intval($config['size']);
		$maxitems = 2;//t3lib_div::intInRange($config['maxitems'],0);
		if (!$maxitems)	$maxitems=100000;
		$minitems = 1;//t3lib_div::intInRange($config['minitems'],0);
