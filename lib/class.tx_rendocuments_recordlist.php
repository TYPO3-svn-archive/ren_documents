 ...
 */
class tx_rendocuments_recordList extends recordList {
		// External:
	...
	function getTable($table,$id,$rowlist)	{
		...
		t3lib_div::loadTCA($table);
			...
			// Init
		$addWhere = ' AND ' . $table . '.workspace = ' . $GLOBALS['SOBE']->id;
				...
				default:			// Regular fields header:
					...
					if ($this->table && is_array($currentIdList))	{
						...
						if (!$TCA[$table]['ctrl']['readOnly'] && $permsEdit && $TCA[$table]['columns'][$fCol])	{
							...
							if ($fCol == 'workspace')
								$params = $GLOBALS['SOBE']->getEditRecordParamsChangeWorkspace($editIdList) . '&columnsOnly='.$fCol.'&disHelp=1';
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ren_documents/lib/class.tx_rendocuments_recordlist.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/ren_documents/lib/class.tx_rendocuments_recordlist.php']);
}
